...
pub mod client {
    ...
    use crate::wasi::http::types::{
        Fields, Method, Scheme, OutgoingRequest, RequestOptions
    };
    ...
    impl HttpClient {
        ...
        pub async fn request(
            ...
            // 4. 处理请求体 (RequestBody) - 标准的 wasi:http 写入流程
            if let Some(data) = body {
                if !data.is_empty() {
                    let request_body = request.body()
                        .map_err(|_| "Failed to get request body resource".to_string())?;
                    
                    let output_stream = request_body.write()
            ...
            let response = response_result
                .map_err(|e| format!("HTTP response error code: {:?}", e))?;
        ...
        fn parse_url(url: &str) -> Result<(Scheme, String, String), String> {
            ...
            let raw_addr = url.trim_start_matches("https://").trim_start_matches("http://");
